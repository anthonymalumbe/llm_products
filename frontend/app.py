...
st.title("📑 Application Assistant powered by RAG")
st.markdown("""
...
def display_validation_results(raw_summary: Optional[str]):
    ...
    # Attempt to find the explicit "Summary" section first
    ...
    if summary_section_match:
        # Take text before the "Summary" line for individual items
        individual_results_text = raw_summary[:summary_section_match.start()]

    # Split individual results by "---" separator
    ...
    entry_pattern = re.compile(
        ...
        r"Status\s*:\s*(.*?)\s*"
        r"Reason\s*:\s*([\s\S]*?)"
        r"(?=\s*(?:[✅❌❓]\s*Field\s*:|\Z))", # Ends with next Field line or end of block
    ...
    for block_content in blocks:
        ...
        if match:
            ...
            # Determine icon based on status text if not captured directly
            current_icon = icon_char if icon_char else "❓" # Default if no icon
        ...
        # else:
            # logger.warning(f"Could not parse validation block: {block_content[:100]}...")


    if results_data:
        df = pd.DataFrame(results_data)
        # Ensure 'No.' column is correctly added and starts from 1
        if not df.empty:
            df.insert(0, 'No.', range(1, 1 + len(df)))


        st.markdown( # CSS for table styling
            """
            <style>
    ...
    if summary_section_match:
    ...
    elif results_data: # Fallback if explicit summary not found
        ...
        summary_cols[1].metric("❌ Failed", failed_count)
        summary_cols[2].metric("❓ Uncertain", uncertain_count)
    # else: # Only if neither summary section nor individual results were found
        # st.warning("Could not parse detailed validation results or summary statistics.")
    ...
    if st.session_state.backend_accessible is None: # Check only once per session or if status is unknown
        try:
            ...
            if response.status_code == 200 and response.json().get("status") == "healthy":
                ...
                # logger.warning(f"Backend health check: FAILED. Status: {response.status_code}, Response: {response.text}")
        except requests.exceptions.ConnectionError:
            st.session_state.backend_accessible = False
            # logger.error(f"Backend health check: ConnectionError to {BACKEND_API_BASE_URL}/health")
        except requests.exceptions.Timeout:
            st.session_state.backend_accessible = False
            # logger.error(f"Backend health check: Timeout connecting to {BACKEND_API_BASE_URL}/health")

    if st.session_state.backend_accessible is False:
        st.error(f"🚨 Cannot connect to the backend API at {RAW_BACKEND_URL}. Please ensure the backend server is running and accessible.")
    # elif st.session_state.backend_accessible is True and not st.sidebar.button("Re-check Backend"): # Show if healthy
        # st.sidebar.success(f"Backend connected at {RAW_BACKEND_URL}")
        # pass
        ...
        if st.button("Process Selected Guidelines", key="process_guidelines_button", type="primary", disabled=st.session_state.backend_accessible is False):
            ...
            with st.spinner("Processing guidelines... This may take a moment."):
                try:
                    ...
                    # st.rerun() # Consider if rerun is always needed
    ...
    if st.button("Validate Application Form", key="validate_application_button", type="primary",
        ...
        with st.spinner(f"Validating '{uploaded_application_file.name}'... This can take some time."):
            try:
                ...
                # st.rerun() # Rerun to display new summary
            except requests.exceptions.RequestException as e:
                err_detail = str(e)
                if hasattr(e, 'response') and e.response is not None:
                    try: err_detail = e.response.json().get("detail", str(e))
                    except: pass # Keep original error if detail parsing fails
...
# --- THIS IS THE NEW CODE BLOCK YOU PROVIDED ---
def get_image_base64(image_path: str) -> Optional[str]:
    ...
    try:
        ...
        # The warning is better handled in the loop that calls this.
        # st.warning(f"Image file not found: {image_path}")
        return None
    except Exception as e:
        st.error(f"Error reading image {image_path}: {e}") # Good to keep this error for debugging
        return None
...
for i, tech in enumerate(tech_logos_data):
    with tech_cols[i]:
        if os.path.exists(tech["img_path"]):
            ...
            if b64_image:
                # Added the caption paragraph back in for completeness
                st.markdown(f""" 
                ...
                """, unsafe_allow_html=True)
            else: # Fallback if base64 conversion failed for some reason
                st.markdown(f"""
                <div class="tech-logo-container">
            ...
            """, unsafe_allow_html=True)
            # Optionally, uncomment for debugging path issues during development:
            # st.warning(f"Logo for {tech['caption']} not found at: {tech['img_path']}")

# --- Footer ---
st.markdown("---")
if st.session_state.backend_accessible is True:
    st.caption(f"Backend API connected: {RAW_BACKEND_URL} (using prefix /api/v1)")
elif st.session_state.backend_accessible is False:
     st.caption(f"Attempted to connect to backend: {RAW_BACKEND_URL} - FAILED")
else:
    st.caption(f"Backend API configured at: {RAW_BACKEND_URL} (status unknown)")